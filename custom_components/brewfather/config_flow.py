...
class BrewfatherConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Brewfather."""
    ...
    @staticmethod
    @callback
    def async_get_options_flow(config_entry: config_entries.ConfigEntry) -> BrewfatherOptionsFlowHandler:
        """Get the options flow for this handler."""
        return BrewfatherOptionsFlowHandler(config_entry)

    async def async_step_user(self, user_input=None):
        """Handle the initial step."""
        errors = {}
        if user_input is not None:
            # Validate the input, if valid, create entry
            # For now, we just assume it's valid and create the entry
            return self.async_create_entry(title="My Brewery", data=user_input)
    ...
    async def async_migrate_entry(self, entry: config_entries.ConfigEntry):
        ...
        if entry.version == 1:
            new_data = {
                CONF_USERNAME: entry.data.get("user_key"),
                CONF_PASSWORD: entry.data.get("api_key"),
            }
            entry.data = new_data
            entry.version = 2
