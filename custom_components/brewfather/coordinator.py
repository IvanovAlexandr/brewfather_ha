...
class BrewfatherCoordinator(DataUpdateCoordinator[BrewfatherCoordinatorData]):
    ...
    def get_active_batches(self):
        """Return list of active batches from the latest data, or empty list."""
        data = getattr(self, "data", None)
        if not data:
            return []
        # The data object may contain a BatchesItem model instance or raw dict
        batches_item = None
        try:
            batches_item = data.get("batches") or data.get("batches_item")
        except Exception:
            batches_item = None

        if batches_item is None:
            # Try to find on top-level attributes
            try:
                batches_item = getattr(data, "batches", None) or getattr(data, "items", None)
            except Exception:
                batches_item = None
        ...
        # If batches_item is a model with helper, use it
        try:
            if hasattr(batches_item, "get_active_batches"):
                return batches_item.get_active_batches()
        except Exception:
            pass

        # Otherwise attempt simple heuristics on raw list/dict
        try:
            candidate = list(batches_item)
        except Exception:
            return []
        ...
        for b in candidate:
            status = None
            try:
                status = getattr(b, "status", None) or getattr(b, "state", None)
            except Exception:
                try:
    ...
    # Ensure coordinator.data includes an explicit list for sensors to iterate
    def get_shared_data_for_entities(self):
        """Return a dict with data convenient for entity setup, including active batches."""
        shared = getattr(self, "data", {}) or {}
        try:
            shared = dict(shared)
        except Exception:
            shared = {}

        shared["active_batches"] = self.get_active_batches()
        return shared
