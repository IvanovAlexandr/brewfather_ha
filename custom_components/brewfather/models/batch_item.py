...
class BatchItem:
    ...
    def get_stage_attributes(self):
        """Return a dict with stage-specific attributes (safe accessors).

        Provides common fields (id, name, stage, status, start/end dates,
        temps, gravities) and serializes readings if present.
        """
        def _get(obj, *names):
            for n in names:
                try:
        ...
        attrs = {}
        attrs["id"] = _get(self, "id", "batch_id")
        attrs["name"] = _get(self, "name", "batch_name")
        attrs["stage"] = _get(self, "stage", "stage_name")
        attrs["status"] = _get(self, "status", "state")
        attrs["start_date"] = _get(self, "start_date", "created_at", "start")
        attrs["end_date"] = _get(self, "end_date", "end_date", "completed_at")
        attrs["target_temp"] = _get(self, "target_temperature", "target_temp")
        attrs["current_temp"] = _get(self, "current_temperature", "temp", "temperature")
        attrs["gravity"] = _get(self, "gravity", "current_gravity", "sg")
        attrs["target_gravity"] = _get(self, "target_gravity", "final_gravity", "fg")
        ...
        if readings:
            ...
            for r in readings:
                try:
                    if hasattr(r, "get_reading_dict"):
                        serial.append(r.get_reading_dict())
                    ...
                    serial.append(r)
            attrs["readings"] = serial

        return attrs

    def to_dict(self):
        """Generic dict representation to help sensors/templates."""
        try:
            # Prefer explicit method if dataclass-like
            d = {}
            ...
            # Fallback: build from known properties
            return self.get_stage_attributes()
